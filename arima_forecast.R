#must run entire code, function is at bottom.  change parameters at bottom next to code, 
...
#call on libaries (if doesnt call you must install)
library(forecast)
library(lmtest)
library(dynlm)
library(useful)
library(quantmod)
getSymbols("^GSPC",periodicity="daily")#load sp500
ticker = GSPC
...
for(i in lookback:nrow(ticker))#align one period forecast into thefuture with current time period. can change lookback period
{
  ...
  arma = auto.arima(data,trace=F)#perform arima
...
lookback = 50#training period for arima model
...
print(c(paste('win ratio :',win.ratio),paste('MSE :',MSE),paste('corr:',correlation),paste('net prof:',net.profit),paste('trades:',trades),paste('return strategy:',100*return.strat),paste('return sp500:',100*return.sp500)))#final results
