...
#data
data<-read.csv("D:/stats projects/johansen test/coint6.csv",header = TRUE)
pair<-read.csv("D:/Trade data/temp/XEL_CMS.csv",header = TRUE)
#ts.plot(data)

stk1 = pair[,1]
stk2 = pair[,2]
pair = cbind(stk1,stk2)#combine the pair
maxlag = 1
max.percent = 90
...
coin<- cointegration.test(stk1=stk1,stk2=stk2,max.precent = max.percent, maxlag = maxlag)
#output
